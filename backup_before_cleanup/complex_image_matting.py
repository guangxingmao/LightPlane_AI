...
class ComplexImageMatting:
    ...
    def _create_adaptive_color_mask(self, image):
        ...
        try:
            ...
            # 自适应阈值
            h_lower = max(0, h_mean - h_std * 0.8)
            h_upper = min(179, h_mean + h_std * 0.8)
            s_lower = max(0, s_mean - s_std * 0.8)
            s_upper = min(255, s_mean + s_std * 0.8)
            v_lower = max(0, v_mean - v_std * 0.8)
            v_upper = min(255, v_mean + v_std * 0.8)
    ...
    def _combine_masks_intelligently(self, masks):
        ...
        try:
            ...
            for i, mask in enumerate(masks[1:], 1):
                if mask is not None:
                    # 智能组合策略
                    combined = cv2.bitwise_and(base_mask, mask)
                    base_mask = cv2.bitwise_or(base_mask, combined)
...
def main():
    """主函数"""
    print("=" * 50)
    print("复杂AI生成图片抠图解决方案")
    print("=" * 50)
    
    # 创建抠图器
    matting = ComplexImageMatting()
    ...
    for i, test_image in enumerate(existing_images):
        ...
        output_path = f"complex_matting_{i+1}_{os.path.basename(test_image)}"
